Public Function qspline(x As Double, xValues As Range, yValues As Range) As Variant
...
If xValues.Rows.Count <> yValues.Rows.Count Then
    qspline = "X & Y Values need to have same number of data points"
    Exit Function
End If
...
If (i = xValues.Rows.Count) Then
    If (xValues(i, 1) < x) Then
        qspline = "Function cannot extrapolate"
        Exit Function
    End If
    qspline = yValues(i, 1)
    Exit Function
...
Dim xTerms(2, 2), yTerms(2, 0) As Double
...
qspline = Coeff(1, 1) + Coeff(2, 1) * x + Coeff(3, 1) * x ^ 2
